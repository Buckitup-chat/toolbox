defmodule Toolbox.OriginLogTest do
  ...
  doctest Toolbox.OriginLog
  alias Toolbox.OriginLog
  ...
  describe "normalize_iolist/1" do
    ...
    test "short charlists are converted to binaries" do
      assert "hi" == OriginLog.normalize_iolist(~c"hi")
    end

    test "longer charlists are kept as lists" do
      assert ~c"long" == OriginLog.normalize_iolist(~c"long")
    end
  end

  describe "use Toolbox.OriginLog" do
    defmodule X do
    ...
    test "injected log/2 function logs with correct prefix" do
      ...
      assert output =~ "[_Toolbox.OriginLogTest.X_]"
